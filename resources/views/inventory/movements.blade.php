@extends('layouts.app')
@section('title', 'حركات المخزون - '.$item->name_ar)
@section('content')

@include('partials.page-header', ['title' => 'حركات: '.$item->name_ar, 'icon' => 'fa-history'])

<div class="flex gap-4 mb-6">
    <a href="{{ route('inventory.stock-in', $item) }}" class="btn-primary text-white px-4 py-2 rounded-lg text-sm flex items-center gap-2">
        <i class="fas fa-arrow-down text-green-400"></i> وارد
    </a>
    <a href="{{ route('inventory.stock-out', $item) }}" class="btn-primary text-white px-4 py-2 rounded-lg text-sm flex items-center gap-2">
            ...
            <tbody class="divide-y divide-slate-800">
                ...
                <tr class="table-row">
                    ...
                    <td class="px-4 py-3 font-bold {{ $m->type==='in'?'text-green-400':'text-red-400' }}">
                        {{ $m->type==='in'?'+':'-' }}{{ number_format($m->quantity, 3) }} {{ $item->unit }}
                    </td>
